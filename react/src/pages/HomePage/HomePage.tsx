...
const lastName="Zhou";
const navList=[{name:"About",link:"/aboutme"},
  {name:"Skills",link:"#skills"},
  {name:"Projects",link:"#projects"},
  {name:"Contact",link:"#contact"}]


    return <div className={isDark?"maxBox-Dark":"maxBox-Light"}>
        <div className={isDark?"logoBox-Dark":"logoBox-Light"}></div>
         ...
         <div className="changeStyleBox">
           ...
           ></div>
             <div className={isDark?"changeButton-Dark":"changeButton-Light"} onClick={handleButtonClick} >
      ...
      >
        {splitName(firstName).map((item,index)=><span key={index}>{item}</span>)}
        <span style={{margin:'0 15px'}}></span>
         {splitName(lastName).map((item,index)=><span key={index}>{item}</span>)}
          
     {/*签名部分*/}
      <div className={`${isDark?"signature-Dark":"signature-Light"} ${isVisible ? "signature-visible" : "signature-hidden"}`}>young and free will climb the hill</div>
    </div>
...
<a href="mailto:your-email@example.com" target="_blank" rel="noopener noreferrer"
  ...
  <span className="iconfont icon-youxiang"></span>
    ...
    <div className={isDark?"githubBt-Dark":"githubBt-Light"}>
      <a href="https://github.com/balancetheworld" target="_blank" rel="noopener noreferrer"
        style={{ textDecoration: 'none' }}  >
