...
const CountPart:React.FC<AvatarProps> = ({isDark}) => {
...
return(
 <div className={isDark?"countBox-Dark":"countBox-Light"} key={index}>
          <span className={isDark?"numBox-Dark":"numBox-Light"}>{count}</span>
          <span className={isDark?"textBox-Dark":"textBox-Light"}>{text}</span>
            </div>
)
}
  ...
  <div className={isDark?"countPart-Dark":"countPart-Light"}>
    {showList.map((item,index)=>{
      return(
         <CountUpBox num={item.num} text={item.txt} index={index}/>
